...
internal class MatchDataRepositoryImpl @Inject constructor(
    ...
    @VisibleForTesting
    internal val matchModelCache = InMemoryKache<String, ActualMatch>(MAX_PAGE_CACHE_SIZE) {
    ...
    override suspend fun getMatch(puuid: String, region: Region, position: Int): MatchWithMeta = withContext(dispatcher) {
        ...
        val matchIdsPage = matchIdPageCache.getOrPut(matchIdPageKey) {
            requestManager.request<MatchIdPage>(matchIdPageRequestFactory.createRequest(matchIdPageKey)).matchIds.takeIf { it.isNotEmpty() }
        } ?: return@withContext MatchWithMeta(match = NoMatch, hasMoreHint = NO_CHANCE)
        val positionOnPage = position % PAGE_SIZE

        if(positionOnPage >= matchIdsPage.size) return@withContext MatchWithMeta(match = NoMatch, hasMoreHint = NO_CHANCE)

        val matchId = matchIdsPage[positionOnPage]
        val hasMoreHint = when {
            matchIdsPage.size < PAGE_SIZE && positionOnPage == matchIdsPage.size.dec() -> NO_CHANCE
            positionOnPage < matchIdsPage.size - 1 -> HasMoreHint.DEFINITELY
            else -> HasMoreHint.LIKELY
        }
        val match = getMatch(matchId, region)
        MatchWithMeta(match = match, hasMoreHint = if (match is NoMatch) NO_CHANCE else hasMoreHint)
    }

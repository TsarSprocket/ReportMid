...
class ProfileOverviewFragment : BaseFragment() {
    ...
    override fun onCreateView(
        ...
        for( i in 0 until TOP_MASTERIES_NUM ) {
            val masteryGroup = layoutInflater.inflate(R.layout.champion_mastery, binding.grpOtherChampMasteries, false )
            binding.grpOtherChampMasteries.addView( masteryGroup )
        }
    ...
    override fun onViewCreated( view: View, savedInstanceState: Bundle? ) {
        ...
        viewModel.activeSummonerModel.observe(viewLifecycleOwner ) { summoner ->
            disposer.add( summoner.icon.observeOn( AndroidSchedulers.mainThread() ).subscribe { icon ->
                binding.imgSummonerIcon.setImageDrawable( icon )
            } )
        ...
        for( i in 0 until TOP_MASTERIES_NUM ) {
            with( viewModel.masteries[ i ] ) {
                shownLive.observe(viewLifecycleOwner ) { fShow ->
                    if( fShow != null ) {
                        binding.grpOtherChampMasteries[ i ].visibility = if( fShow ) View.VISIBLE else View.GONE
                    }
                }
                iconLive.observe(viewLifecycleOwner ) { icon ->
                    if( icon != null ) {
                        binding.grpOtherChampMasteries[i].findViewWithTag<ImageView>( resources.getString(R.string.fragment_profile_overview_tag_champion_icon) ).setImageDrawable( icon )
                    }
                }
/*
...
*/
                skillsLive.observe(viewLifecycleOwner ) { skills ->
                    if( skills != null ) {
                        with(binding.grpOtherChampMasteries[i]) {

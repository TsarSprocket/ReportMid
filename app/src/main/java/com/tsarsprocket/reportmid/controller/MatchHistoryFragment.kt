...
class MatchHistoryFragment : BaseFragment() {
    ...
    private val matchHistoryAdapter by lazy {
        MatchHistoryPagingAdapter(
            MatchComparator(),
            iconProvider
        )
    }
    ...
    override fun onCreateView(
        ...
        binding =
            DataBindingUtil.inflate(inflater, R.layout.fragment_match_history, container, false)
        ...
        with(binding.bottomNavigation.menu) {
            for(i in 0 until size()) with(get(i)) {
                if(id != R.id.matchHistoryFragment) isEnabled = true
            }
        }
    ...
    class MatchHistoryPagingAdapter(
        diffCallback: DiffUtil.ItemCallback<MatchHistoryModel.MyMatch>,
                        ...
                        with(holder) {
                            with(CardMatchHistoryBinding.bind(cardView)) {
                                ...
                                match.blueTeam.participants.plus(match.redTeam.participants)
                                    ...
                                    ?.let { myParticipant ->
                                        colourStripe.setBackgroundColor(
                                        ...
                                        with(holder) {
                                            disposer.addAll(
                                                iconProvider.getChampionIcon(myParticipant.champion.iconName).observeOn(
                                                    AndroidSchedulers.mainThread()
                                                )
                                                    .subscribe { drawable ->
                                                        imgChampionIcon.setImageDrawable(
                                                            drawable
                                                        )
                                                    },
                                                iconProvider.getSummonerSpellIcon(myParticipant.summonerSpellD.iconName).observeOn(
                                                    AndroidSchedulers.mainThread()
                                                )
                                                    .subscribe { drawable ->
                                                        iconSummonerSpellD.setImageDrawable(
                                                            drawable
                                                        )
                                                    },
                                                iconProvider.getSummonerSpellIcon(myParticipant.summonerSpellF.iconName).observeOn(
                                                    AndroidSchedulers.mainThread()
                                                )
                                                    .subscribe { drawable ->
                                                        iconSummonerSpellF.setImageDrawable(
                                                            drawable
                                                        )
                                                    },
                                            )
                                            myParticipant.primaryRune?.run { iconProvider.getRuneIcon(iconName) }?.observeOn(
                                                AndroidSchedulers.mainThread()
                                            )
                                                ?.subscribe { drawable ->
                                                    iconPrimaryRune.setImageDrawable(
                                                        drawable
                                                    )
                                                }?.let { disposer.add(it) }
                                            myParticipant.secondaryRunePath?.let {
                                                iconProvider.getRunePathIcon(
                                                    it.iconPath
                                                )
                                            }?.observeOn(AndroidSchedulers.mainThread())
                                                ?.subscribe { drawable ->
                                                    iconSecondaryRunePath.setImageDrawable(
                                                        drawable
                                                    )
                                                }?.let { disposer.addAll() }
                                        }
                                        ...
                                        txtGameMode.text =
                                            cardView.resources.getString(match.gameType.titleResId)
                                        txtMainKDA.text =
                                            "${myParticipant.kills}/${myParticipant.deaths}/${myParticipant.assists}"
                                        txtCS.text = "CS: ${myParticipant.creepScore}"

                                        myParticipant.items.zip(itemViews)
                                            .forEach { (item, imageView) ->
                                                if(item != null) {
                                                    holder.disposer.add(
                                                        iconProvider.getItemIcon(item.imageName)
                                                            .observeOn(AndroidSchedulers.mainThread())
                                                            .subscribe { drawable ->
        ...
        override fun onCreateViewHolder(
            parent: ViewGroup,
            viewType: Int
        ): CardViewHolderWithDisposer = CardViewHolderWithDisposer(

...
import com.tsarsprocket.reportmid.matchUpView.impl.view.Error
...
import com.tsarsprocket.reportmid.matchUpView.impl.view.NotFound
    ...
    override fun Visualize(modifier: Modifier, state: ViewState, stateHolder: ViewStateHolder) {
        if(state is InternalViewState) {
            when(state) {
                ...
                is ErrorState -> Error(modifier, state, stateHolder)
                is NotFoundState -> NotFound(modifier, state, stateHolder)
            }
        } else {

...
internal class InteractorImpl @Inject constructor(
    ...
    override suspend fun getSummoner(puuid: String, region: Region): Summoner {
        val summoner = summonerRepository.requestRemoteSummonerByPuuidAndRegion(PuuidAndRegion(Puuid(puuid), region))
        return Summoner(summonerLevel = summoner.level)
    }
